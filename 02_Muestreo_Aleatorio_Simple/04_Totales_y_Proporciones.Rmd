...
limite_inferior <- 17.01433 - z*ee
limite_superior <- 17.01433 + z*ee
c(limite_inferior,limite_superior)
...
**Ejericio** ¿Qué precisión se requiere para que la muestra sea de tamaño n=100?. Alternativa a solución, usar función buscar objetivo en excel.

**Ejercicio** Considera el factor de ajuste por población finita y calcula nuevamente el intervalo de confianza anterior s considerar el factor de ajuste por población finita $1 - \frac{n}{N}$.
...
```{r}
# Error Estándar
ee <- 14.21652	 / sqrt(148)
ee
```


```{r}
limite_inferior <- 17.85833 - z*ee
limite_superior <- 17.85833 + z*ee
c(limite_inferior,limite_superior)
...
```{r}
limite_inferior <- 0.285 - z*ee
limite_superior <- 0.285 + z*ee
c(limite_inferior,limite_superior)
...
Para la muestra elegida, el intervalo sí cntiene el parámetro poblacional.
